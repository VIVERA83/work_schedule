...
class WorkScheduleHistoryCreateSchema(BaseModel):
    ...
    id_schedule_type: int = Field(default="идентификатор типа расписания:", examples=["1"], gt=0)
    date: datetime = Field(
        default=datetime(year=datetime.now().year, month=datetime.now().month, day=datetime.now().day),
        description="дата начала графика")
    is_working: bool = Field(default=True, description="флаг работы водителя")
    what_day: int = Field(default=1, description="Который день, водитель работает(отдыхает).", gt=0)
